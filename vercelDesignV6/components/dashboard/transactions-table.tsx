...
function StatusBadge({ status }: { status: "completed" | "pending" | "failed" }) {
  ...
  if (status === "pending") {
    return (
      <Badge
        variant="outline"
        className="gap-1 border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-400"
      >
  ...
  return (
      <Badge
        variant="outline"
        className="gap-1 border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-400"
      >
...
export function TransactionsTable() {
  return (
    <div className="rounded-xl border border-border bg-card shadow-sm transition-all duration-200 hover:border-[#0c7c80]/50 hover:shadow-md dark:hover:border-[#17b5ba]/40">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border/50">
            {transactions.map((tx) => (
              <tr
                key={tx.id}
                className="transition-colors hover:bg-[#dce8f5] dark:hover:bg-[#122848]"
              >
