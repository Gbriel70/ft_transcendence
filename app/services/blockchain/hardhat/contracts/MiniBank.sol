...
contract MiniBank
{
    ...
    event WalletRegistered(address indexed wallet, uint256 indexed userId, uint256 timestamp);
    ...
    {
        require(isRegistered[wallet], "Wallet not registered");
        _;
    }
    ...
    // Chamado pelo backend ao criar usuário — credita 100 tokens automaticamente
    function registerWallet(uint256 userId, address wallet) external onlyOwner
    {
        ...
        balances[wallet]     = INITIAL_BALANCE;
        totalUsers++;

        emit WalletRegistered(wallet, userId, block.timestamp);
    }

    // Transferência entre usuários registrados
